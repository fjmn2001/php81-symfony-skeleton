...
UID = $(shell id -u)
DOCKER_BE = medine-php81-symfony54
...
start: ## Start the containers
	docker network create medine-php81-symfony54-network || true
...
logs: ## Show Symfony logs in real time
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} symfony serve:log

composer-install: ## Install composer dependencies
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} composer install --no-interaction
